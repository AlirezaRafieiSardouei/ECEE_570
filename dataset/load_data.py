...
def create_dataset(
    ...
    if args.DATA_TYPE == 'UHDM':
        ...
        aim_files = sorted([file for file in os.listdir(data_path + '/dimmed') if file.endswith('.jpg')])
...
class aim_data_loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_in = number # + '.jpg'
        if self.mode == 'train':
            path_tar = self.args.TRAIN_DATASET + '/bright/' + image_in_gt
            path_src = self.args.TRAIN_DATASET + '/dimmed/' + image_in
            if self.loader == 'crop':
                x = random.randint(0, 1024 - self.args.CROP_SIZE)
                y = random.randint(0, 1024 - self.args.CROP_SIZE)
                labels, moire_imgs = crop_loader(self.ImageType, self.args.CROP_SIZE, x, y, [path_tar, path_src])

            elif self.loader == 'resize':
                labels, moire_imgs = resize_loader(self.ImageType, self.args.RESIZE_SIZE, [path_tar, path_src])
                data['origin_label'] = default_loader([path_tar])[0]

            elif self.loader == 'default':
                labels, moire_imgs = default_loader(self.ImageType, [path_tar, path_src])

        elif self.mode == 'test':
            image_in = number # + '.png'
            image_in_gt = number # + '.png'
            path_tar = self.args.TEST_DATASET + '/bright/' + image_in_gt
            path_src = self.args.TEST_DATASET + '/dimmed/' + image_in
            if self.loader == 'resize':
                labels, moire_imgs = resize_loader(self.ImageType, self.args.RESIZE_SIZE, [path_tar, path_src])
                data['origin_label'] = default_loader(self.ImageType, [path_tar])[0]
            else:
                labels, moire_imgs = default_loader(self.ImageType, [path_tar, path_src])
...
def default_loader(ImageType, path_set=[]):
    imgs = []
    for path in path_set:
         img = Image.open(path).convert(ImageType)
         # img = img.load()
         # img = Image.merge("RGB", (img[2,:,:], img[2,:,:], img[2,:,:]))
         img = default_toTensor(img)
         imgs.append(img)
    
    return imgs
    
def crop_loader(ImageType, crop_size, x, y, path_set=[]):
    imgs = []
    for path in path_set:

         img = Image.open(path).convert(ImageType)
         # h, s, v = img.split()
         # img = Image.merge("RGB", (v, v, v))
...
def resize_loader(ImageType, resize_size, path_set=[]):
    imgs = []
    for path in path_set:
         img = Image.open(path).convert(ImageType)
         img = img.load()
